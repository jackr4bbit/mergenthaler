...
def build(feed: Feed, output: Path, nodelete: bool = False):
    if not nodelete and output.exists():
...
def serve(location: Path, port: int = 8080, host: str = "0.0.0.0"):
    app = Flask(__name__)
...
def main():
    ...
    match args.command:
        case "build":
            ...
            build(feed, outputPath, args.nodelete)

        case "serve":
            print(f"Starting server at {args.host}:{args.port}!")
            serve(inputPath, args.port, args.host)
